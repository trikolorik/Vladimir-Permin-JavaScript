...
function getGoods() {
    fetch('db/db.json').then(res => res.json()).then(result => {
        allGoods = result;
    });
}
...
function scrollTop() {
    const scrollLinks = document.querySelector('a.scroll-link');
...
function createCard(objCard) {
    ...
    <div class='goods-card'>
        ...
        <img src="db/${objCard.img}" alt="db/${objCard.name}" class="goods-image">
        <h3 class="goods-title">${objCard.name}</h3>
        <p class="goods-description">${objCard.description}</p>
        <button class="button goods-card-btn add-to-cart" data-id="${objCard.id}"><span class="button-price">${objCard.price}$
        </span></button>
    <div>
    `;
